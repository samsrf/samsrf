function Analyse_Simulated_Prfs
%
% Fits a standard 2D Gaussian pRF model to simulated pRF data. 
%
...
load aps_pRF % Load apertures to simulate responses for
% Simulate pRF data
Srf = samsrf_simulate_prfs(Ground_Truth, @(P,ApWidth) prf_gaussian_rf(P(1), P(2), P(3), ApWidth), ApFrm); % Simulate time courses
save('sim_pRF', 'Srf'); % Save simulated data 
...
Model.Aperture_File = 'aps_pRF'; % Box standard sweeping bars design we typically use
...
%% Fit pRF model
MapFile = samsrf_fit_prf(Model, 'sim_pRF');
