...
% Search grid (1 and 2 in reverse order because MatLab is stupid)
[S2, S1, S3, S4, S5] = ndgrid(Param2, Param1, Param3, Param4, Param5);
S = [S1(:) S2(:) S3(:) S4(:) S5(:)]';

% Generating predictions
if wb 
    h = waitbar(0,'Generating predictions...');
end
for n = 1:numel(S1)
    Rfp = PrfFcn([S1(n) S2(n) S3(n) S4(n) S5(n)], size(ApFrm,1)*2); % pRF profile
    cptc = prf_predict_timecourse(Rfp, ApFrm, false);
    Ptc(:,n) = cptc; 
    if wb
        waitbar(n/numel(S1), h);
    end
end
if wb
    close(h);
