   ...
   case 'samsrf_fit_prf'
       switch ParameterName
           ...
           case 'Replace_Bad_Fits'
               HelpText = { '[Optional] Boolean' 
							''
							'If true, then any fine fits that do not meet the criteria for a good fit will be replaced by the original coarse fit parameters. We have not used this feature very much but it could be a way to deal with odd misestimations in the fine-fit (which seem more likely in Nelder-Mead algorithm - see description of fitting algorithms in Cookbook).' 
							''
							'Defaults to false.' };
   ...
   case 'samsrf_revcor_prf'
       switch ParameterName
           ...
           case 'Rdim'
               HelpText = { '[Optional] Scalar'
							''
							'If you decide to save them in the map file (see Save_Rmaps) this defines the side length of reverse correlation profiles. It is probably unnecessary to save those in the resolution of apertures. A smaller number means you use a lot of disc space.'
							''
							'Defaults to 50.' };
           case 'Noise_Ceiling_Threshold'
               HelpText = { '[Optional] Scalar' 
           ...
           case 'Save_Rmaps'
               HelpText = { '[Optional] Boolean'
							...
							''
							'All other analyses are possible without saving these profiles because they can be easily calculated again.'
							''
							'Defaults to true.' };
           case 'Hooke_Jeeves_Steps'
               HelpText = { '[Optional] Vector of scalars'
       ...
       switch ParameterName
           ...
           case 'Fit_pRF'
               HelpText = { '[Optional] Boolean'
							...
							''
							'By default, this is not defined meaning that no 2D model is fit & CF parameters are only roughly estimated from the reverse correlation profiles.' };
           case 'Hooke_Jeeves_Steps'
               HelpText = { '[Optional] Vector of scalars'
							''
							'This defines for each pRF parameter the initial step size to be used for the Hooke-Jeeves (parameter search) algorithm. If this is defined, the 2D model fit uses this algorithm instead of the standard Nelder-Mead simplex search algorithm (fminsearch) - so be certain you want this!'
